...
const resolveSimpleAuthBaseUrl = () => {
    try {
        ...
        console.warn('Unable to determine SimpleAuthClient base URL, using relative paths.', error);
    }
    return '';
};
...
class SimpleAuthClient {
    constructor() {
        this.baseURL = resolveSimpleAuthBaseUrl();
    ...
    async resendVerificationCode(email) {
        try {
            const response = await fetch(this.buildUrl('/api/auth/send-verification'), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async forgotPassword(email) {
        try {
            const response = await fetch(this.buildUrl('/api/auth/send-password-reset'), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
